var Audio = {
    ...
    if (!list || !list.length) {
      if (__cur.shownAudios == 0 && (__cur.album_id || (!Audio.allAudios(from_pad).length && !__cur.searchStr))) {
        ...
        if (Audio.allAudios(from_pad).length) {
          ...
          if (__cur.oid > 0 && !__cur.audioFriend) {
  ...
  searchAudios: function(opts) {
    ...
    if (from_pad && !__cur.audiosIndex) {
      Audio.loadFriendsAudios({from_pad: from_pad, id: vk.id, index: 'all', str: str});
      return;
    }
    ...
    if (str) {
      ...
      audioPlayer.genPlaylist(__cur.audiosList[__cur.curList], false, {my: (__cur.curList == 'all' && (__cur.oid == vk.id || from_pad) ? 'my' : false), htitle: __cur.justShuffled && window.audioPlaylist && audioPlaylist.htitle});
  ...
  changeAllIndex: function(opts) {
    ...
    var __cur = opts.from_pad ? window._pads && _pads.cur : window.cur,
        ...
        showAlbums = opts.showAlbums, owner = opts.owner, str = opts.str || '';
  ...
  addShareAudio: function(opts) {
    ...
    var __cur = opts.from_pad ? window._pads && _pads.cur : window.cur,
        ...
        from_choose = opts.from_choose, ev = opts.ev;
    ...
    if (__cur.curSection == 'recommendations') query.recommendation = 1;
    ...
    ajax.post(Audio.address, query, {
      onDone: function (data, res) {
        ...
        obj = obj['all'][0];
        if (((!cur.gid && (cur.id == vk.id || from_pad)) || (cur.gid && gid)) && obj && cur.module == 'audio') {
          setTimeout(function(){
            if (cur.audiosIndex) {
  ...
  loadFriendsAudios: function(opts) {
    opts = opts || {};
    var __cur = opts.from_pad ? window._pads && _pads.cur : window.cur,
        from_pad = !!opts.from_pad, id = opts.id, index = opts.index, str = opts.str,
    ...
    if (!__cur.audiosList[index]) {
      ...
      ajax.post(Audio.address, query, {
        ...
        onDone: function(data, options) {
          ...
          Audio.changeAllIndex({from_pad: from_pad, index: index, album: album, showAlbums: showAlbums, owner: owner, str: str});
