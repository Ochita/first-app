...
<div class="im_unread_bar">\
  ...
  addMsg: function(peer_id, after_id, msg_id, status, out, title, message, date, kludges, delayed) {
    ...
    if (!out && status > 1 && cur.notify_on && cur.focused != peer_id) {
  ...
  updateDialog: function(peer, lastMsg, unreadCnt, cnts) {
    ...
    // Inline author
    if (!srvMsg && (lastMsg[2] || kludges.from)) {
      if (!lastMsg[2]) {
...
//            IM.filterFriends(); // bad idea to shuffle friends, when you filter them
  ...
  restoreDraft: function(peer) {
    ...
    if (curv.length < draftv.length) {
      if (cur.editable) {
        val(txt, Emoji.emojiToHTML(clean(draftv), true).replace(/\n/g, '<br/>'));
        Emoji.editableFocus(txt, false, true);
      } else {
        val(txt, draftv);
      }
  ...
  init: function(options) {
    ...
    if (options.text_data || options.media_data) {
      ...
      if (cur.editable) {
        val(txt, Emoji.emojiToHTML(clean(text), true).replace(/\n/g, '<br/>'));
        Emoji.editableFocus(txt, false, true);
      } else {
        val(txt, text);
      }
...
//              frag.appendChild(srch);
  ...
  wrapChats: function (list, to_match) {
    ...
    for (var i in chat_list) {
      ...
      var txt = '<img src="' + ((cur.chat_photos || {})[i] || '/images/icons/multichat.png') + '" class="fl_l" width="32" height="32"/><div class="fl_l"><nobr>' + name + '</nobr></div>';
  ...
  restoreWriteDraft: function () {
    ...
    if (draft = ls.get('im_draft' + vk.id + '_' + peer)) {
      if (Emoji.editableVal(txt).length < 2) {
        if (cur.editable) {
          val(txt, Emoji.emojiToHTML(clean(draft.txt || ''), true).replace(/\n/g, '<br/>'));
        } else {
          val(txt, draft.txt || '');
        }
      }
  ...
  initWrite: function() {
    ...
    debugLog('Ahhh', cur.peer); // should be -3
    ...
    /*if (cur.editable) {
      addEvent(IM.getNewTxt(), 'paste', function() {
        Emoji.onEditablePaste(IM.getNewTxt(), IM.checkEditable);
      });
