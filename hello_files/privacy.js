var Privacy = {
  ...
  someSaved: function(key, ids, list, plain) {
    ...
    for (var i = 0; i < count && i < 5; ++i) {
      ...
      str.push('<a href="/' + (shortname ? shortname : ('id' + id)) + '" onclick="return nav.go(this, event)">' + mem[1] + '</a>');
    }
    str = str.join(', ');
    if (count > 5) {
...
extend(OList.prototype, {
  ...
  renderList: function (pattern, offset, sel) {
    ...
    if (!offset && !html.length) {
      html.push('<div class="olist_empty">' + (pattern ? getLang('global_search_not_found').replace('{search}', clean(pattern)) : self.noSelMsg) + '</div>');
    }
    re(this.moreEl);
    html = html.join(' ');
